...
get_instrument_configs <- function() {
  ...
  instrument_configs <- list(
    astral = list(
      ...
      max_scan_rate = 100,   # Hz - conservative for stability
      optimal_scan_rate = 50, # Hz - optimal for data quality
      min_scan_rate = 10,    # Hz - minimum practical scan rate
      parallel_acquisition = TRUE,
      cycle_calculation = "ms2_based",
    ...
    orbitrap = list(
      ...
      ms2_time = 50.0,       # ms
      max_scan_rate = 12,    # Hz
      optimal_scan_rate = 8,  # Hz - optimal for data quality
      min_scan_rate = 2,     # Hz - minimum practical scan rate
      parallel_acquisition = FALSE,
      cycle_calculation = "traditional",
    ...
    orbitrap_exploris = list(
      ...
      ms2_time = 22.0,       # ms
      max_scan_rate = 40,    # Hz
      optimal_scan_rate = 25, # Hz - optimal for data quality
      min_scan_rate = 5,     # Hz - minimum practical scan rate
      parallel_acquisition = FALSE,
      cycle_calculation = "traditional",
    ...
    timstof_pro = list(
      ...
      ms1_time = 10.0,       # ms
      ms2_time = 1.5,        # ms
      max_scan_rate = 120,   # Hz
      optimal_scan_rate = 60, # Hz - optimal for data quality
      min_scan_rate = 10,    # Hz - minimum practical scan rate
      parallel_acquisition = TRUE,
      cycle_calculation = "ms2_based",
    ...
    sciex_7600 = list(
      ...
      ms2_time = 10.0,       # ms
      max_scan_rate = 50,    # Hz
      optimal_scan_rate = 30, # Hz - optimal for data quality
      min_scan_rate = 5,     # Hz - minimum practical scan rate
      parallel_acquisition = FALSE,
      cycle_calculation = "traditional",
    ...
    waters_synapt = list(
      ...
      ms2_time = 20.0,       # ms
      max_scan_rate = 20,    # Hz
      optimal_scan_rate = 12, # Hz - optimal for data quality
      min_scan_rate = 3,     # Hz - minimum practical scan rate
      parallel_acquisition = FALSE,
      cycle_calculation = "traditional",
    ...
    custom = list(
      ...
      optimal_scan_rate = 12, # Hz (user configurable)
      min_scan_rate = 2,     # Hz (user configurable)
      parallel_acquisition = FALSE,
      cycle_calculation = "traditional",
...
list_instruments <- function() {
  ...
  instrument_df <- data.frame(
    ...
    Parallel = sapply(configs, function(x) x$parallel_acquisition),
...
print_instrument_info <- function(instrument_name) {
  ...
  cat(sprintf("Parallel acquisition: %s\n", ifelse(config$parallel_acquisition, "Yes", "No")))
...
validate_instrument_config <- function(config) {
  ...
  if (!config$cycle_calculation %in% c("ms2_based", "traditional")) {
    errors <- c(errors, "Cycle calculation must be 'ms2_based' or 'traditional'")
    valid <- FALSE
  }
