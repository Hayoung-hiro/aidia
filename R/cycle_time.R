# cycle_time.R - Scan / cycle time computation for DIA experiments
#
# Purpose: All scan timing and cycle time math, including parallel-instrument
#          duty cycle sync. Covers Orbitrap, Astral, and TOF analyzers.
# Split from the legacy instrument_utils.R in v0.4.1.
#
# See docs/adr/0001-keep-parallel-sync-logic-in-cycle-time.md for the
# rationale on keeping parallel sync logic in this module.


# =============================================================================
# Constants
# =============================================================================
# Analyzer timing constants and fallback defaults moved to
# R/cycle_time_constants.R (v0.4.x) to keep this module focused on computation
# and parallel-sync logic. They remain in the package namespace (all R/*.R load
# together), so references below resolve normally. See that file for
# ORBITRAP_TRANSIENT_TIME_MS, the ASTRAL_* parameters, ORBITRAP_240K_TRANSIENT_MS,
# DEFAULT_MS1_OVERHEAD_MS, DEFAULT_OVERHEAD_FACTOR, and MINIMUM_OVERHEAD_MS.
...
# =============================================================================
# Simple Cycle Time Helper
# =============================================================================

#' Compute Cycle Time from Pre-Computed MS1 / Total MS2 Times
#'
#' Lightweight alternative to \code{calculate_cycle_time_from_experiment()}
#' for call sites that already have MS1 scan time and total MS2 time in hand
#' (e.g., inside IT optimization loops). Dispatches on cycle mode:
#' \itemize{
#'   \item \code{"parallel"}: \code{max(ms1_time, total_ms2_time)}
#'   \item \code{"sequential"}: \code{ms1_time + total_ms2_time}
#' }
#'
#' Use this instead of inlining \code{max(...)} or \code{+} so the cycle time
#' formula has a single canonical implementation in this file.
#'
#' @param ms1_time Numeric, MS1 scan time (any consistent unit; sec or ms).
#' @param total_ms2_time Numeric, total MS2 time (same unit as \code{ms1_time}).
#'   Typically \code{n_windows * single_ms2_scan_time}.
#' @param cycle_mode Character, either \code{"parallel"} or \code{"sequential"}.
#'
#' @return Numeric, cycle time in the same unit as the inputs.
#' @export
#' @keywords internal
#'
#' @examples
#' \dontrun{
#' # Astral (parallel) at 51 windows
#' simple_cycle_time(ms1_time = 0.266, total_ms2_time = 0.255, "parallel")
#'
#' # Exploris (sequential)
#' simple_cycle_time(ms1_time = 0.122, total_ms2_time = 1.6, "sequential")
#' }
simple_cycle_time <- function(ms1_time, total_ms2_time, cycle_mode) {
  if (!cycle_mode %in% c("parallel", "sequential")) {
    stop(sprintf("cycle_mode must be 'parallel' or 'sequential', got '%s'",
                 cycle_mode))
  }
  if (cycle_mode == "parallel") {
    max(ms1_time, total_ms2_time)
  } else {
    ms1_time + total_ms2_time
  }
}
