...
# This represents the actual chromatographic peak width used in DPPP calculations
...
calculate_dppp <- function(fwhm_minutes, cycle_time_seconds, instrument = "astral") {
  ...
  # Calculate DPPP using Spectronaut's definition: peak width = 1.7 × FWHM
...
calculate_cycle_time_from_dppp <- function(target_dppp, mean_fwhm) {
  # Reference formula with Spectronaut's definition: cycle_time = (1.7 × mean_fwhm / target_dppp) × 1000
  peak_width_seconds <- mean_fwhm * PEAK_WIDTH_FACTOR
  cycle_time_ms <- (peak_width_seconds / target_dppp) * 1000
