...
#' Plot Window Width Distribution (Alternative: Single Faceted Version)
#'
#' Simpler faceted version for quick overview
#'
#' @param optimized_windows OptimizedWindows object from Stage 3
#' @param validated_data ValidatedData object from Stage 1
#' @param max_segments_to_show Maximum number of RT segments to display
#'
#' @return ggplot object
#' @export
plot_window_width_distribution_faceted <- function(optimized_windows,
                                                     validated_data,
                                                     max_segments_to_show = 6) {

  cat("  Generating Plot 7 (Faceted): Window Width Distribution...\n")

  # Extract data
  windows_data <- optimized_windows$windows
  precursor_data <- validated_data$data

  # Extract RT bin information
  rt_bins <- windows_data %>%
    group_by(rt_segment_id) %>%
    summarize(
      rt_start = first(rt_start),
  ...
  rt_bins_filtered <- rt_bins %>%
    filter(rt_segment_id %in% selected_segments) %>%
    mutate(rt_label = sprintf("RT%02d: %.1f-%.1f min",
                             rt_segment_id, rt_start, rt_end))

  # Prepare precursor data with RT labels
  precursor_annotated <- precursor_data %>%
    left_join(
      rt_bins_filtered %>% select(rt_segment_id, rt_start, rt_end, rt_label),
      by = character()
    ) %>%
    filter(
      RT.Apex >= rt_start,
  ...
  # Prepare window data
  windows_annotated <- windows_data %>%
    filter(rt_segment_id %in% selected_segments) %>%
    left_join(rt_bins_filtered %>% select(rt_segment_id, rt_label),
              by = "rt_segment_id")
  ...
  p <- ggplot() +
    # Density curve
    geom_density(
      data = precursor_annotated,
      aes(x = Precursor.Mz, y = after_stat(scaled)),
      color = "steelblue",
      fill = "steelblue",
      alpha = 0.3,
      linewidth = 0.8
    ) +
    # Window width as segments
    geom_segment(
      data = windows_annotated,
      aes(x = mz_start, xend = mz_end, y = 0, yend = 0),
      color = "coral",
      linewidth = 2,
      alpha = 0.7
    ) +
    facet_wrap(~ rt_label, ncol = 3, scales = "free_x") +
    labs(
      title = "Window Width Distribution by RT Segment",
      subtitle = "Blue = Normalized precursor density | Red = Window coverage",
      x = "m/z (Da)",
